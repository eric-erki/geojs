...
$(function () {
  ...
  var map = geo.map({
    node: '#map',
    center: {
      x: query.x ? +query.x : -75.150,
      y: query.y ? +query.y : 43.712
    },
    zoom: query.zoom ? +query.zoom : 5,
  ...
  var nlcd = map.createLayer('osm', {keepLower: false, attribution: null});

  nlcd.url(
    function (x, y, zoom) {
      var baseUrl = 'http://localhost:8090/nlcd/' + zoom + '/' + x + '/' + y + ".png";
      return baseUrl;
    }
  );
